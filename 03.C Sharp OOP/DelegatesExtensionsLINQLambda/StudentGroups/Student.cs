namespace StudentGroups
{
    using System;
    using System.Collections.Generic;
    using System.Text;
    using System.Text.RegularExpressions;

    public class Student
    {
        private string firstName;
        private string lastName;
        private string facultyNumber;
        private string tel;
        private string email;
        private List<int> marks;
        private int groupNumber;


        public Student(string firstName, string lastName, string facultyNumber, string tel,
            string email, int groupNumber, params double[] inputMarks)
        {
            this.FirstName = firstName;
            this.LastName = lastName;
            this.FacultyNumber = facultyNumber;
            this.Telephone = tel;
            this.Email = email;
            this.GroupNumber = groupNumber;
            this.Marks = new List<double>(inputMarks);
        }

        public Student(string firstName, params double[] inputMarks)
            : this(firstName, "Peshev", "15203006", "+359888888888", "pesho.peshev@gmail.com", 2, 2)
        {
        }

        public Student()
            : this("Pesho", "Peshev", "159001", "+359888888888", "pesho.peshev@gmail.com", 2, 2)
        {
        }

        public Group Group { get; set; }

        public string FirstName
        {
            get { return this.firstName; }
            private set
            {
                if (string.IsNullOrEmpty(value))
                {
                    throw new ArgumentException("First name can not be blank.");
                }

                this.firstName = value;
            }
        }

        public string LastName
        {
            get { return this.lastName; }
            private set
            {
                if (string.IsNullOrEmpty(value))
                {
                    throw new ArgumentException("Last name can not be blank.");
                }

                this.lastName = value;
            }
        }

        public string FacultyNumber
        {
            get { return this.facultyNumber; }
            private set
            {
                if (string.IsNullOrEmpty(value))
                {
                    throw new ArgumentException("Faculty number can not be blank.");
                }

                this.facultyNumber = value;
            }
        }

        public string Telephone
        {
            get { return this.tel; }
            set
            {
                if (string.IsNullOrEmpty(value))
                {
                    throw new ArgumentException("Telephone number can not be blank.");
                }
                if (!Regex.IsMatch(value, @"(\+359[0-9]{9})") && !Regex.IsMatch(value, @"(0[0-9]{9})"))
                {
                    throw new ArgumentException("Telephone numbers begin with +359 or 0 followed by 9 digits.");
                }

                this.tel = value;
            }
        }

        public string Email
        {
            get { return this.email; }
            set
            {
                if (string.IsNullOrEmpty(value))
                {
                    throw new ArgumentNullException("Email can not be blank.");
                }
                if (!Regex.IsMatch(value, @"(\w+@\w+.\w+)"))
                {
                    throw new ApplicationException("Email must be valid.");
                }

                this.email = value;
            }
        }

        public int GroupNumber
        {
            get { return this.groupNumber; }
            set
            {
                if (value <= 0)
                {
                    throw new ApplicationException("Group number must be bigger than 0.");
                }

                this.groupNumber = value;
            }
        }

        public List<double> Marks { get; set; }

        public void AddMark(int currentMark)
        {
            if (currentMark < 2 || currentMark > 6)
            {
                throw new ApplicationException("Marks are between 2 and 6.");
            }

            this.marks.Add(currentMark);
        }


        public void RemoveMarkAtPosition(int position)
        {
            if (position < 0 || position >= this.marks.Count)
            {
                throw new ApplicationException("No such position.");
            }

            this.marks.RemoveAt(position);
        }

        public override string ToString()
        {
            var sb = new StringBuilder();

            var properties = this.GetType().GetProperties();

            sb.AppendLine(new string('-', 30));

            foreach (var property in properties)
            {
                if (property.Name == "Marks" || property.Name == "Group")
                {
                    continue;
                }

                sb.AppendFormat("{0}: {1}", property.Name.PadLeft(15), property.GetValue(this, null));
                sb.AppendLine();
            }

            sb.Append("Marks: ".PadLeft(17));
            sb.AppendFormat(string.Join(",", this.Marks));

            return sb.ToString();
        }
    }
}
